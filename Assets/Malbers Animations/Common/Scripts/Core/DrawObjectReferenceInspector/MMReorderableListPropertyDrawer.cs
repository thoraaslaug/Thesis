...
{
    public static class MMReorderableListPropertyDrawer
    {
        ...
        public static void Draw(SerializedProperty serializedProperty, Action<SerializedProperty, Rect, int, bool, bool> elementDrawCallback)
        {
            ...
            {
                EditorGUI.LabelField(new Rect(rect.x, rect.y, rect.width, rect.height), $"{serializedProperty.displayName} [{list.count}]");
            };
            list.drawElementCallback = (rect, index, active, focused) =>
            {
                elementDrawCallback(serializedProperty, new Rect(rect.x + 10, rect.y, rect.width, rect.height), index, active, focused);
            };
